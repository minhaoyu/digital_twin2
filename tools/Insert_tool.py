        ...
        with open(path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                kwargs = {
                    ...
                    "power_factor": row[13] if row[13] else 0,
                    "frequency": row[14] if row[14] else 0,
                    "real_energy_imported": row[15] if row[15] else 0,
                    "real_energy_exported": row[16] if row[16] else 0,
                    "reactive_energy_imported": row[17] if row[17] else 0,
                    "reactive_energy_exported": row[18] if row[18] else 0,
                    "apparent_energy": row[19] if row[19] else 0,
                    "energy_price": row[20] if row[20] else 0,
                    "raise_6_sec_price": row[21] if row[21] else 0,
                    "raise_60_sec_price": row[22] if row[22] else 0,
                    "raise_5_min_price": row[23] if row[23] else 0,
                    "date_time": date_time,
                }
